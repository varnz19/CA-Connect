...
export const generateInvoicePdf = (invoice: any): Promise<Buffer> => {
  return new Promise((resolve, reject) => {
    try {
      ...
      // Logo box
      const logoW = 80;
      const logoPath = path.join(__dirname, '../../assets/ca-logo.png');
      ...
      doc.text('Invoice Number:', leftColX, my).font('Helvetica-Bold').text(invoice.invoiceNumber, leftColX + 90, my); my += 10;
      doc.font('Helvetica').text('Invoice Date:', leftColX, my).font('Helvetica-Bold').text(issueDateStr, leftColX + 90, my); my += 10;
      doc.font('Helvetica').text('Due date:', leftColX, my).font('Helvetica-Bold').text(dueDateStr, leftColX + 90, my); my += 10;
      doc.font('Helvetica').text('Place of Supply:', leftColX, my).font('Helvetica-Bold').text(clientState, leftColX + 90, my); my += 10;
      doc.font('Helvetica').text('Reverse Charge:', leftColX, my).font('Helvetica-Bold').text('No', leftColX + 90, my); my += 10;
      doc.font('Helvetica').text('Optional Field 1:', leftColX, my).font('Helvetica').text('-', leftColX + 90, my); my += 10;
      doc.font('Helvetica').text('Optional Field 2:', leftColX, my).font('Helvetica').text('-', leftColX + 90, my); my += 10;
      ...
      doc.fontSize(8.5).font('Helvetica-Bold').text('Transporter Details:', rightColX, my); my += 10;
      doc.font('Helvetica').text('Transporter:', rightColX, my).font('Helvetica-Bold').text('Direct Transportation / Portal', rightColX + 100, my); my += 10;
      doc.font('Helvetica').text('Vehicle No:', rightColX, my).font('Helvetica-Bold').text('MH01CA1234', rightColX + 100, my); my += 10;
      doc.font('Helvetica').text('Transporter Doc No:', rightColX, my).font('Helvetica-Bold').text(`DOC/CAC/${invoice.invoiceNumber.replace(/[^0-9]/g, '') || '1234'}`, rightColX + 100, my); my += 10;
      doc.font('Helvetica').text('Transporter Doc Date:', rightColX, my).font('Helvetica-Bold').text(issueDateStr, rightColX + 100, my); my += 10;
      doc.font('Helvetica').text('E-Way Bill No:', rightColX, my).font('Helvetica-Bold').text('271234567890', rightColX + 100, my); my += 10;
      ...
      my = currY + 4;
      doc.fontSize(9).font('Helvetica-Bold').text('Billing Details', leftColX, my, { underline: true }); my += 12;
      doc.fontSize(8.5).font('Helvetica-Bold').text(`Name: ${clientName}`, leftColX, my); my += 10;
      doc.font('Helvetica').text(`GSTIN: ${clientGstin} | Mobile: ${clientPhone}`, leftColX, my); my += 10;
      doc.text(`Email: ${clientEmail}`, leftColX, my); my += 10;
      doc.text(`Add Address: ${clientAddress}`, leftColX, my);

      // Shipping Details (Right)
      my = currY + 4;
      doc.fontSize(9).font('Helvetica-Bold').text('Shipping Details', rightColX, my, { underline: true }); my += 12;
      doc.fontSize(8.5).font('Helvetica-Bold').text(`Name: ${clientName}`, rightColX, my); my += 10;
      doc.font('Helvetica').text(`GSTIN: ${clientGstin} | Mobile: ${clientPhone}`, rightColX, my); my += 10;
      doc.text(`Email: ${clientEmail}`, rightColX, my); my += 10;
      ...
      doc
        ...
        .font('Helvetica-Bold')
        .text(
          `PO/Ref No: PO-CAC-${invoice.invoiceNumber.replace(/[^0-9]/g, '') || '2026'}   |   Job No: CAC/JOB-4820   |   Job Date: ${issueDateStr}`,
          boxX + 6,
          currY + 4
        );
      ...
      doc.text(
        `Settled by - Bank: Rs. ${isPaid ? total.toFixed(2) : '0.00'}  |  Invoice Balance: Rs. ${isPaid ? '0.00' : total.toFixed(2)}`,
        boxX + 6,
        currY + 3
      );
      doc.text(
        `Tax @${taxRate}% : Rs. ${subtotal.toFixed(2)}  |  CGST (${halfRate}%): Rs. ${cgst.toFixed(2)}  |  SGST (${halfRate}%): Rs. ${sgst.toFixed(2)}  |  Total Tax: Rs. ${(cgst + sgst).toFixed(2)}  |  Add. Cess: 0.00`,
        boxX + 6,
        currY + 12
      );
      currY += 23;
